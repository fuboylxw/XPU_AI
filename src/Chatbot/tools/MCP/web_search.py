...
class WebSearchTool:
    ...
    async def search_baidu(
        ...
        try:
            ...
            if self.session:
                ...
                # 同步请求作为备用，使用线程池避免阻塞事件循环
                loop = asyncio.get_event_loop()
                def _req_get():
                    return requests.get(url, headers=headers, timeout=10)
                resp = await loop.run_in_executor(None, _req_get)
                if resp.status_code == 200:
                    return self._parse_baidu_results(resp.text, max_results)
    ...
    async def search_baidu_ai(
        self, query: str, max_results: int = 5, search_recency_filter: str = "day"
    ) -> List[Dict[str, Any]]:
        """使用百度AI搜索API进行搜索"""
        try:
            ...
            # 添加调试信息 - 只记录最小必要信息
            self.logger.debug(f"百度AI搜索请求URL: {url}")
            self.logger.debug(f"百度AI搜索请求payload类型: {type(payload).__name__}")
            self.logger.debug(
                f"百度AI搜索Token前缀: {self.settings.baidu_ai_search_token[:5]}..."
            )
            ...
            if self.session:
                ...
                resp = await loop.run_in_executor(None, _req_post)
                if resp.status_code == 200:
                    try:
                        result = resp.json()
                    except Exception:
                    ...
                    self.logger.error(f"百度AI搜索API请求失败: {resp.status_code}")
                    self.logger.error(f"错误响应: {resp.text}")
    ...
    def _parse_baidu_ai_results(
        self, api_result: Dict[str, Any], max_results: int
    ) -> List[Dict[str, Any]]:
        """解析百度AI搜索API结果"""
        try:
            results = []
            ...
            # 添加调试信息 - 只记录核心部分
            self.logger.debug(f"百度AI搜索API返回结果长度: {len(str(api_result))}")

            # 检查API响应结构 - 处理references字段
            if "references" in api_result and isinstance(
                api_result["references"], list
            ):
                ...
                for i, item in enumerate(references[:max_results]):
                    try:
                        ...
                        if result["title"] and result["url"]:
                            ...
                            self.logger.debug(f"成功解析第 {i+1} 个结果")
            ...
            self.logger.debug(f"百度AI搜索解析完成，返回 {len(results)} 个结果")
            return results
            ...
            for i, item in enumerate(items[:max_results]):
                try:
                    ...
                    if title and url:
                        ...
                        print(
                            f"❌ 第 {i+1} 个结果缺少标题或链接: title={bool(title)}, url={bool(url)}"
                        )
    ...
    async def comprehensive_search(
        self, query: str, max_results: int = 5, engines: List[str] = None
    ) -> List[Dict[str, Any]]:
        """综合搜索 - 仅使用百度"""
        return await self.search_with_fallback(query, max_results)

    async def search_and_extract(
        self, query: str, max_results: int = 3, engines: List[str] = None
    ) -> List[Dict[str, Any]]:
        """搜索并提取内容 - 仅使用百度"""
    ...
    def search_sync(
        self, query: str, engine: str = "auto", max_results: int = 5
    ) -> List[Dict[str, Any]]:
        """同步搜索方法 - 仅使用百度"""
        if self._should_skip_engine("baidu"):
            self.logger.warning("百度搜索频率限制，跳过搜索")
    ...
    def search(self, query: str, max_results: int = 5) -> str:
        ...
        try:
            # 确保百度AI搜索Token已配置
            if not self.settings.baidu_ai_search_token:
                self.logger.error("百度AI搜索Token未配置")
                return "百度AI搜索未配置，请检查配置文件中的SEARCH_ENGINE_API_KEY"

            self.logger.info(f"使用百度AI搜索: {query}")
            ...
            # 添加调试日志 - 只记录核心部分
            self.logger.debug(f"百度AI搜索URL: {url}")
            self.logger.debug(
                f"百度AI搜索Token前缀: {self.settings.baidu_ai_search_token[:5]}..."
            )
            ...
            if response.status_code == 200:
                ...
                ai_results = self._parse_baidu_ai_results(result, max_results)
                ...
                if ai_results:
                    ...
                    self.logger.warning("百度AI搜索未返回结果")
                    return "百度AI搜索未返回结果，请稍后再试"
            else:
                self.logger.error(f"百度AI搜索请求失败: {response.status_code}")
                self.logger.error(f"错误响应: {response.text}")
                return "百度AI搜索请求失败，请检查网络连接和API配置"

        except Exception as e:
            self.logger.error(f"百度AI搜索失败: {e}")
            return f"百度AI搜索出错: {str(e)}"
