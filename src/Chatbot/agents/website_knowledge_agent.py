...
"""
    ...
    def classify_text(self, text: str) -> str:
        ...
        # 教务管理
            ...
            "课程" in text or "教务" in text or "考试" in text or "选课" in text or "学分" in text
        ):
            return "教务管理"
        # 学生事务
        elif (
        ...
        # 规章制度
            ...
            "规章" in text or "制度" in text or "办法" in text or "规定" in text or "文件" in text
        ):
            return "规章制度"
        # 就业指导
        elif (
            "就业" in text or "实习" in text or "职业" in text or "招聘" in text or "工作" in text
        ):
            return "就业指导"
        # 新闻动态
        elif (
            "新闻" in text or "动态" in text or "通知" in text or "公告" in text or "活动" in text
        ):
            return "新闻动态"
        # 默认分类
        else:
    ...
    def extract_content(self, url: str, html_content: str) -> Dict[str, Any]:
        ...
        categorized_content = {
            category: [] for category in self.knowledge_categories.keys()
        }
        ...
        sections = soup.find_all(
            ["div", "section", "article"], class_=lambda c: c and len(c) > 5
        )
        ...
        for category in categorized_content:
            for i, content in enumerate(categorized_content[category]):
                if isinstance(content, bytes):
                    categorized_content[category][i] = content.decode(
                        ...
                        if contents:  # 只有当该类别有内容时才添加
                            ...
                            for content in contents:
                                if content:
                                    ...
                                    category_contents[category].append(
                                        source_info + "\n" + content
                                    )
            ...
            for category, contents in category_contents.items():
                ...
                with open(file_path, "w", encoding="utf-8-sig") as f:
                    ...
                    if category in self.knowledge_categories:
                        f.write(
                            f"描述: {self.knowledge_categories[category]['description']}\n\n"
                        )
...
def main():
    ...
    parser.add_argument("--output", "-o", default="knowledge_base", help="知识库输出目录")
    parser.add_argument("--max-pages", "-m", type=int, default=100, help="最大爬取页面数")
    parser.add_argument("--delay", "-d", type=float, default=1.0, help="爬取延迟(秒)")
