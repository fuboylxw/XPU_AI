...
try:
    ...
    logger.warning("PaddleOCR not installed. Please install with: pip install paddleocr")
    raise
...
try:
    ...
    logger.warning("FAISS not installed. Please install with: pip install faiss-cpu")
    raise
...
try:
    ...
    logger.warning("LangChain not installed. Please install with: pip install langchain")
    raise
    ...
    def _initialize_ocr(self):
        ...
        try:
            ...
            try:
                import paddle
                try:
                    use_gpu = bool(paddle.device.cuda.device_count() > 0)
                except Exception:
                    use_gpu = False
            except Exception:
                use_gpu = False
            ...
            if self.faiss_index is None:
                ...
                try:
                    try:
                        ...
                        torch = None
                    if torch and torch.cuda.is_available() and hasattr(faiss, "StandardGpuResources"):
                        res = faiss.StandardGpuResources()
                        self.faiss_index = faiss.index_cpu_to_gpu(res, 0, self.faiss_index)
                        logger.info("FAISS索引已迁移到GPU")
                except Exception:
                    pass
