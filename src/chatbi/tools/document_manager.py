...
class DocumentManager:
    ...
    def __init__(self):
        ...
        self.logger = setup_logger("document_manager")
    ...
    def _generate_document_id(self, content, filename):
        """生成文档ID"""
        import hashlib
    ...
    def reload_data(self):
        """重新加载文档数据"""
        self.logger.info("尝试重新加载文档数据")
        try:
            # 检查文档目录
            documents_dir = self.settings.documents_dir
            if not documents_dir.exists():
                self.logger.error(f"文档目录不存在: {documents_dir}")
                return False
            ...
            for file_path in documents_dir.glob("**/*.txt"):
                try:
                    ...
                    # 提取类别（从路径中）
                    category = file_path.parent.name if file_path.parent.name != documents_dir.name else "其他"
                    
                    # 生成文档ID
                    doc_id = self._generate_document_id(content, file_path.name)
            ...
            for doc in documents:
                ...
                self.document_metadata[doc_id] = {
                    ...
                    'end_idx': len(all_chunks) + len(chunks)
                }
            ...
            if all_chunks:
                ...
                self.logger.info(f"创建了包含 {self.vector_db.ntotal} 个向量的数据库")
                
                # 保存数据
                self._save_data()
                return True
            else:
                self.logger.warning("没有文本块可以向量化")
                return False
        except Exception as e:
            self.logger.error(f"重新加载文档数据失败: {e}")
            return False
    ...
    async def search_documents(self, query: str, category: Optional[str] = None, top_k: int = None) -> List[Dict[str, Any]]:
        ...
        if not self.vector_db or self.vector_db.ntotal == 0:
            self.logger.warning("文档向量数据库为空，尝试重新加载数据")
            self.reload_data()
            
            # 再次检查
            if not self.vector_db or self.vector_db.ntotal == 0:
                self.logger.error("无法加载文档数据")
                return []
        ...
        try:
            ...
            results = []
            # 使用默认相似度阈值，以防settings中没有定义
            similarity_threshold = getattr(self.settings, 'similarity_threshold', 0.5)
