...
try:
    ...
    # 检查TTS是否可用
    if not tts_agent.api_key:
        logger.error("TTS API密钥未配置！请在.env文件中配置TTS_API_KEY")
        raise HTTPException(status_code=500, detail="TTS服务未配置")
    else:
        logger.info(f"TTS代理初始化成功，API密钥已配置: {tts_agent.api_key[:20]}...")
        
except Exception as e:
    logger.error(f"TTS代理初始化失败: {e}")
    raise HTTPException(status_code=500, detail=f"TTS服务初始化失败: {str(e)}")
            ...
            try:
                ...
                if temp_file_path and os.path.exists(temp_file_path):
                    try:
                        ...
                        logger.warning(f"删除语音识别临时文件失败: {temp_file_path}, 错误: {e}")
                        # 尝试强制删除
                        try:
                            import time
                            time.sleep(0.1)  # 短暂等待
...
async def voice_phone_call(
    audio_file: UploadFile = File(...),
    user_id: str = Form("phone_user"),
    db: Session = Depends(get_db)
) -> StreamingResponse:
    """
    语音电话接口 - 完整的语音对话流程，支持连续对话
    1. 接收音频文件并进行语音识别
    2. 获取或创建电话会话，维护独立的对话上下文
    3. 调用聊天智能体处理识别的文本
    4. 将回复转换为语音并流式返回
    
    参数:
    - audio_file: 音频文件
    - user_id: 用户ID（可选，默认为phone_user）
    """
    try:
        # 验证文件类型
        if not audio_file.content_type or not audio_file.content_type.startswith('audio/'):
        ...
        logger.info(f"开始处理语音电话请求，用户ID: {user_id}，音频大小: {len(audio_data)} bytes")
        
        # 获取或创建电话会话
        session_id = get_or_create_phone_session(user_id, db)
        
        # 第一步：语音识别
        # 创建临时文件保存音频，使用上下文管理器确保文件被正确关闭
        temp_file_path = None
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.wav') as temp_file:
                temp_file_path = temp_file.name
                ...
                try:
                    ...
                    logger.warning("语音识别无法理解音频")
                    recognized_text = "抱歉，我没有听清楚您说的话，请重新说一遍。"
                except sr.RequestError as e:
                    logger.error(f"语音识别服务错误: {e}")
                    recognized_text = "语音识别服务暂时不可用，请稍后再试。"
                except Exception as e:
                    logger.error(f"语音识别异常: {e}")
                    recognized_text = "语音识别出现问题，请重新尝试。"
                
                # 第二步：获取电话对话历史，构建上下文
                logger.info(f"获取电话会话 {session_id} 的对话历史")
                conversation_history = get_phone_conversation_history(session_id, db, limit=5)
                
                # 构建上下文字符串
                context_messages = []
                for history in conversation_history:
                    context_messages.append(f"用户: {history.question}")
                    context_messages.append(f"助手: {history.answer}")
                
                context_str = "\n".join(context_messages) if context_messages else ""
                
                # 第三步：调用聊天智能体处理文本
                logger.info(f"调用聊天智能体处理文本: {recognized_text}")
                ...
                chat_response = await chat_agent.answer_question(
                    ...
                    user_id=user_id,
                    user_role="guest",
                    conversation_id=session_id,  # 使用电话会话ID
                    phone_mode=True,  # 启用电话模式优化
                    fast_response=True  # 启用快速响应
                )
                
                if chat_response.get("success", False):
                    response_text = chat_response.get("answer", "抱歉，我无法回答您的问题。")
                    logger.info(f"[DEBUG] 成功获取chat_response，answer字段: {response_text[:200]}...")
                else:
                    response_text = "抱歉，系统暂时无法处理您的请求，请稍后再试。"
                    logger.warning(f"[DEBUG] chat_response失败，使用默认回复: {response_text}")
                    logger.warning(f"[DEBUG] chat_response内容: {chat_response}")
                
                logger.info(f"聊天智能体回复: {response_text[:100]}...")
                logger.info(f"[DEBUG] 即将用于TTS的完整文本: {response_text}")
                
                # 保存电话对话记录
                save_phone_conversation(session_id, user_id, recognized_text, response_text, db)
                ...
                def generate_audio_stream() -> Generator[bytes, None, None]:
                    ...
                    try:
                        logger.info("开始生成语音流")
                        logger.info(f"[DEBUG] TTS输入文本: {response_text}")
                        logger.info(f"[DEBUG] TTS输入文本长度: {len(response_text)}")
                        ...
                        logger.error(f"生成语音流失败: {e}")
                        # 返回错误提示音频（如果可能的话）
                        error_text = "抱歉，语音合成出现问题。"
                        logger.info(f"[DEBUG] 使用错误提示文本: {error_text}")
                        try:
                            for chunk in tts_agent.synthesize_speech_stream(error_text):
                                if chunk:
                                    yield chunk
                        except:
                            # 如果连错误音频都无法生成，返回空数据
                ...
                return StreamingResponse(
                    ...
                    headers={
                        "Content-Disposition": "attachment; filename=phone_response.mp3",
                        "Cache-Control": "no-cache",
                        "Connection": "keep-alive",
                        "X-Phone-Session-ID": session_id,  # 返回电话会话ID给前端
                        "X-Recognized-Text": recognized_text[:100].encode('utf-8', errors='ignore').decode('latin-1', errors='ignore'),  # 安全编码
                        "X-Response-Text": response_text[:100].encode('utf-8', errors='ignore').decode('latin-1', errors='ignore')       # 安全编码
                    }
                )
                
        except Exception as inner_e:
            logger.error(f"语音处理内部错误: {inner_e}")
            raise
        finally:
            # 确保临时文件被删除
            if temp_file_path and os.path.exists(temp_file_path):
                try:
                    ...
                    try:
                        ...
                        time.sleep(0.1)  # 短暂等待
                        os.unlink(temp_file_path)
                        logger.info(f"延迟删除临时文件成功: {temp_file_path}")
                    except Exception as e2:
                        logger.error(f"强制删除临时文件也失败: {temp_file_path}, 错误: {e2}")
                        # 记录需要手动清理的文件
                        logger.error(f"需要手动清理的临时文件: {temp_file_path}")
            
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"语音电话处理失败: {str(e)}")
        raise HTTPException(status_code=500, detail=f"语音电话处理失败: {str(e)}")


@router.post("/voice/phone_call_async/")
    ...
    try:
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.wav') as temp_file:
                ...
                try:
                    recognized_text = recognizer.recognize_google(audio, language='zh-CN')
                    logger.info(f"异步语音识别成功: {recognized_text}")
                except:
                ...
                return StreamingResponse(
                    ...
                    media_type="audio/mpeg",
                    headers={
                        "Content-Disposition": "attachment; filename=phone_response_async.mp3",
                        "Cache-Control": "no-cache",
                        "Connection": "keep-alive",
            ...
            if temp_file_path and os.path.exists(temp_file_path):
                try:
                    ...
                    logger.warning(f"删除异步临时文件失败: {temp_file_path}, 错误: {e}")
                    # 尝试强制删除
                    try:
                        import time
                        time.sleep(0.1)  # 短暂等待
                        ...
                        # 记录需要手动清理的文件
                        logger.error(f"需要手动清理的异步临时文件: {temp_file_path}")
...
async def get_phone_call_status() -> Dict[str, Any]:
    ...
    try:
        ...
        status = {
            ...
            "text_to_speech": {
                "available": hasattr(tts_agent, 'api_key') and bool(getattr(tts_agent, 'api_key', True)),
            ...
            "response_format": "audio/mpeg (MP3)",
            "features": [
